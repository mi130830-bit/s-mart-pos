...
Future<void> main() async {
  ...
  try {
    ...
    final countRes = await conn.execute(
      ...
      WHERE name LIKE '%ยาแนว%tb%' 
         OR name LIKE '%ยาแนว%TB%'
         OR name LIKE '%tb%ยาแนว%'
         OR name LIKE '%TB%ยาแนว%'
      """
    );
    final count = int.parse(countRes.rows.first.colByName('cnt')!);
    stdout.writeln('Number of products containing "ยาแนว" and "tb": $count');

    if (count == 0) {
      stdout.writeln('No matching products found. No update needed.');
      return;
    }
    ...
    final updateRes = await conn.execute(
      """
      UPDATE product SET reorderPoint = 5.0 
      WHERE name LIKE '%ยาแนว%tb%' 
         OR name LIKE '%ยาแนว%TB%'
         OR name LIKE '%tb%ยาแนว%'
         OR name LIKE '%TB%ยาแนว%'
    ...
    await conn.execute(
      "INSERT INTO activity_log (action, details) VALUES (:action, :details)",
      {
        ...
        'details': 'อัปเดตจุดสั่งซื้อขั้นต่ำ (reorderPoint) เป็น 5 สำหรับสินค้ากลุ่มยาแนว TB ทั้งหมด จำนวน $count รายการ'
      }
    );
