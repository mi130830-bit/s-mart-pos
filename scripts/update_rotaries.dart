...
Future<void> main() async {
  ...
  try {
    ...
    final countRes = await conn.execute(
      "SELECT COUNT(*) as cnt FROM product WHERE name LIKE 'ดอกโรตารี่%'"
    );
    final count = int.parse(countRes.rows.first.colByName('cnt')!);
    stdout.writeln('Number of products starting with "ดอกโรตารี่": $count');

    if (count == 0) {
      stdout.writeln('No products start with "ดอกโรตารี่". No update needed.');
      return;
    }

    // Perform the update
    final updateRes = await conn.execute(
      "UPDATE product SET productType = :typeId WHERE name LIKE 'ดอกโรตารี่%'",
      {'typeId': drillTypeId}
    );
    ...
    await conn.execute(
      "INSERT INTO activity_log (action, details) VALUES (:action, :details)",
      {
        ...
        'details': 'เปลี่ยนประเภทสินค้าเป็น ดอกสว่าน (ID: $drillTypeId) สำหรับสินค้าที่ขึ้นต้นด้วย "ดอกโรตารี่" ทั้งหมด จำนวน $count รายการ'
      }
    );
