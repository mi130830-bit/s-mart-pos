...
class Product {
  ...
  final bool isActive;
  final bool isWarehouseItem; 
  final bool hasComponents; // ✅ Feature: Linked products indicator
  final bool hasPriceTiers; // ✅ Feature: Tier prices indicator
  final bool hasExtraUnits; // ✅ Feature: Extra units indicator

  bool get isComposite => components != null && components!.isNotEmpty || hasComponents;

  // ✅ Decimal Helpers for Precise Calculation
  Decimal get costPriceDecimal => Decimal.parse(costPrice.toString());
  Decimal get retailPriceDecimal => Decimal.parse(retailPrice.toString());
  Decimal get wholesalePriceDecimal =>
      Decimal.parse((wholesalePrice ?? 0.0).toString());
  Decimal get memberRetailPriceDecimal =>
      Decimal.parse((memberRetailPrice ?? 0.0).toString());
  Decimal get memberWholesalePriceDecimal =>
      Decimal.parse((memberWholesalePrice ?? 0.0).toString());
  ...
  Product({
    ...
    this.priceTiers,
    this.isActive = true,
    this.isWarehouseItem = false,
    this.hasComponents = false,
    this.hasPriceTiers = false,
    this.hasExtraUnits = false,
  });
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    ...
    return Product(
      ...
      isWarehouseItem: parseBool(json['isWarehouseItem']), 
      hasComponents: parseBool(json['hasComponents']),
      hasPriceTiers: parseBool(json['hasPriceTiers']),
      hasExtraUnits: parseBool(json['hasExtraUnits']),
    );
  }
  ...
  Product copyWith({
    ...
    return Product(
      ...
      isWarehouseItem: isWarehouseItem ?? this.isWarehouseItem,
      hasComponents: hasComponents ?? this.hasComponents,
      hasPriceTiers: hasPriceTiers ?? this.hasPriceTiers,
      hasExtraUnits: hasExtraUnits ?? this.hasExtraUnits,
    );
  }
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'isWarehouseItem': isWarehouseItem ? 1 : 0,
      'hasComponents': hasComponents ? 1 : 0,
      'hasPriceTiers': hasPriceTiers ? 1 : 0,
      'hasExtraUnits': hasExtraUnits ? 1 : 0,
    };
  }
}
