  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      if (_logoPath != null) {
        ...
        // ✅ Save as Base64 for syncing to other clients
        try {
          final file = File(_logoPath!);
          if (await file.exists()) {
            final bytes = await file.readAsBytes();
            final base64String = base64Encode(bytes);
            await settings.set('shop_logo_base64', base64String);
          }
        } catch (e) {
          debugPrint('Error saving logo base64: $e');
        }
      }
      ...
      if (mounted) {
        AlertService.show(
          context: context,
          message: 'บันทึกข้อมูลไม่สำเร็จ: $e',
          type: 'error',
        );
      }
    }
  ...
  Future<void> _pickLogo() async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        setState(() {
          // ✅ Fix: Replace Backslash with Forward Slash to avoid MySQL escape issues
          _logoPath = result.files.single.path!.replaceAll(r'\', '/');
        });
      }
    } catch (e) {
