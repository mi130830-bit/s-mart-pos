...
class DeliveryHistoryRepository {
  ...
  Future<void> initTable() async {
    try {
      ...
      // Attempt schema upgrades for existing tables safely
      await _db.ensureColumn('delivery_history', 'locationUrl', 'TEXT NULL');
      await _db.ensureColumn('delivery_history', 'customerPhone', 'VARCHAR(50) NULL');
      await _db.ensureColumn('delivery_history', 'distanceKm', 'DECIMAL(8,2) DEFAULT 0.0');
      await _db.ensureColumn('delivery_history', 'fuelCostEstimate', 'DECIMAL(10,2) DEFAULT 0.0');
      // ✅ New columns for full data capture from S-Link
      await _db.ensureColumn('delivery_history', 'billImageUrl', 'TEXT NULL');
      await _db.ensureColumn('delivery_history', 'destinationLat', 'DECIMAL(10,7) NULL');
      await _db.ensureColumn('delivery_history', 'destinationLng', 'DECIMAL(10,7) NULL');
