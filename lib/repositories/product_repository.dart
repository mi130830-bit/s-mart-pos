...
class ProductRepository {
  ...
  Future<List<Product>> getAllProducts({bool forceRefresh = false}) async {
    // ✅ [CHANGED] MySQL-First Strategy เพื่อป้องกันราคาเก่าค้างใน Cache
    // เดิม: Isar → MySQL (เสี่ยงราคาเก่าทำให้ขาดทุน)
    // ใหม่: MySQL → อัปเดต Isar → Fallback Isar ถ้าออฟไลน์

    // 1. ลอง MySQL ก่อนเสมอ
    ...
    if (_dbService.isConnected()) {
      try {
        ...
        // ✅ Self-heal: อัปเดต Isar Cache ให้ตรงกับ MySQL เสมอ
    ...
    // 2. Fallback: Isar (ออฟไลน์ หรือ MySQL ล่ม)
    // ⚠️ ราคาอาจไม่ใช่ล่าสุด — ยอมรับได้เฉพาะกรณีออฟไลน์จริงๆ
    try {
    ...
    try {
      ...
      if (_dbService.isConnected()) {
        ...
        if (rows.isNotEmpty) {
          ...
          // ✅ NEW: Apply Enhancements (Composite Stock, Tiers, Barcodes)
  ...
  Future<List<Product>> getProductsPaginated(int page, int pageSize,
    ...
    try {
      ...
      // ✅ Relevance Sort เมื่อมีคำค้นหา → ตรงเป๊ะก่อน → ขึ้นต้น → contains
      // + Natural Sort ด้วย LENGTH(name) เพื่อให้ แม่สี1 ขึ้นก่อน แม่สี10, แม่สี11
      if (searchTerm != null && searchTerm.isNotEmpty) {
        sql += '''
 ORDER BY
   CASE
 ...
 LIMIT :limit OFFSET :offset''';
      ...
      if (products.isNotEmpty) {
        ...
        // ดึงข้อมูลส่วนประกอบและเพิ่มฟิลด์เสริมของสินค้าเหล่านี้
  ...
  // ✅ Centralized method to calculate composite stock and check extra features
  Future<List<Product>> _applyProductEnhancements(List<Product> products) async {
    ...
    try {
      ...
      // 2. Tiers and Extra Units Checks
