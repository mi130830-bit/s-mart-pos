...
class RedemptionRecord {
  ...
  factory RedemptionRecord.fromJson(Map<String, dynamic> json) {
    ...
    final name =
        '$fname $lname'.trim().isNotEmpty ? '$fname $lname'.trim() : 'ลูกค้า';
...
class RewardRepository {
  ...
  Future<void> initTable() async {
    ...
    await _dbService.ensureColumn(
        'point_reward', 'reward_type', "VARCHAR(10) DEFAULT 'GIFT'");
    await _dbService.ensureColumn(
        'point_reward', 'discount_value', 'DECIMAL(10,2) DEFAULT 0');
    await _dbService.ensureColumn(
        'point_reward', 'coupon_expiry_days', 'INT DEFAULT 30');
    await _dbService.ensureColumn(
        'reward_redemption', 'reward_type', "VARCHAR(10) DEFAULT 'GIFT'");
  ...
  Future<bool> deleteReward(int id) async {
    ...
    try {
      final result = await _dbService
          .execute('DELETE FROM point_reward WHERE id = :id', {'id': id});
      return result.affectedRows > BigInt.zero;
    } catch (e) {
  ...
  Future<CouponValidationResult> validateCoupon(String code,
    ...
    try {
      ...
      if (status == 'USED') {
        return CouponValidationResult.invalid('คูปองนี้ถูกใช้ไปแล้ว');
      }
      if (status == 'EXPIRED') {
