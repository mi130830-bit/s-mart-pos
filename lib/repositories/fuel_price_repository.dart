...
class FuelPriceRepository {
  ...
  Future<Map<String, double>> buildPriceLookup(
      DateTime start, DateTime end) async {
    // ใช้วิธีดึงทั้งหมดแทน เพื่อให้มี fallback ไปยังราคาแรกสุด หรือล่าสุดได้เสมอ
    // (ข้อมูล fuel_prices มีจำนวนน้อยมาก ไม่กระทบ performance)
  ...
  static double resolvePriceFromLookup(
    ...
    // 🔥 Fallback: ถ้าหาวันที่ก่อนหน้าไม่เจอเลย ให้ใช้วันที่เก่าที่สุดที่มีในระบบ (เพื่อคำนวณย้อนหลังได้)
    if (bestKey == null && sortedKeys.isNotEmpty) {
      bestKey = sortedKeys.first;
    }
