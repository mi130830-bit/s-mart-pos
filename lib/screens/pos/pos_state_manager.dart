...
class PosStateManager extends ChangeNotifier {
  ...
  Promotion? _appliedPromotion; // ✅ โปรที่ถูก apply อยู่
  ...
  double _vatRate = 7.0; // ✅ VAT Rate from Settings
  ...
  Future<void> refreshGeneralSettings() async {
    ...
    _vatRate = settings.vatRate; // ✅ Load VAT Rate
    _allowPriceEdit =
        settings.getBool('allow_pos_price_edit', defaultValue: false);

    // ✅ Reload promotions เพื่อให้ตรงกับสิ่งที่เพิ่ม/แก้ไขใน session
  ...
  PriceCalculationResult get _calcResult {
    return _calcCache ??= _priceCalcService.calculateTotals(
      ...
      vatRate: _vatRate, // ✅ ส่ง VAT Rate จาก Settings
    );
  }
  ...
  double get discountAmount =>
      ...
      _calcResult.extraDiscountAmount.toDouble() + // ✅ Extra Discount
    ...
    for (final req in requests) {
      ...
      try {
        ...
        await _cartService.addProduct(
          ...
          overridePrice: Decimal.zero, // ฟรี! ราคา 0
          customer: _currentCustomer,
          tier: currentTier,
        );
        // ใส่หมายเหตุแถมฟรีในรายการที่เพิ่มล่าสุด
        final newIndex = cart.length - 1;
        if (newIndex >= 0) {
          _cartService.updateItemComment(
              newIndex, "🎁 แถมฟรี (${_appliedPromotion?.name ?? 'โปรโมชั่น'})");
        }
        debugPrint('🎁 Added free item: ${product.name} x${req.quantity}');
      } catch (e) {
  ...
  Future<void> addProductToCart(Product product,
    ...
    // ✅ Re-fetch fresh stock from DB ก่อนตรวจสอบ Stock เสมอ
    // ป้องกัน Stale Cache: สต็อกที่ติดมากับ Product Object อาจเก่า
    // เช่น พนักงานเปิด Dialog ค้างไว้ แล้วเพิ่มสต็อกในอีกหน้าจอ
