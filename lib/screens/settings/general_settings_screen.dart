...
class _GeneralSettingsScreenState extends State<GeneralSettingsScreen> {
  ...
  bool _enableWarehouseAutoTag = true; // ✅ New State
  String _roundingMode = 'none';
  String _itemDiscountMode = 'per_item'; // ✅ New State
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _enableWarehouseAutoTag = settings.enableWarehouseAutoTag; // ✅ Load Value
      _roundingMode = settings.roundingMode;
      _itemDiscountMode = settings.itemDiscountMode; // ✅ Load Value
  ...
  Future<void> _saveSettings() async {
    ...
    await settings.set(
        'enable_warehouse_auto_tag', _enableWarehouseAutoTag); // ✅ Save Value
    await settings.set('rounding_mode', _roundingMode);
    await settings.set('item_discount_mode', _itemDiscountMode); // ✅ Save Value
      ...
      body: _isLoading
          ...
          : ListView(
              ...
              children: [
                ...
                Card(
                  child: Column(
                    children: [
                      ...
                      SwitchListTile(
                        title: const Text(
                            'คัดกรองสินค้าหลังร้านอัตโนมัติ (Warehouse Auto Tag)'),
                        subtitle: const Text(
                    ...
                    children: [
                      ListTile(
                        ...
                        trailing: DropdownButton<String>(
                          ...
                          items: const [
                            DropdownMenuItem(
                                value: 'none', child: Text('ไม่ปัดเศษ (ตามจริง)')),
                            DropdownMenuItem(
                                value: 'satang_25', child: Text('ปัดอัตโนมัติ (เต็ม 25 สตางค์)')),
                            DropdownMenuItem(
                                value: 'auto', child: Text('ปัดอัตโนมัติ (เต็มบาท)')),
                            DropdownMenuItem(
                                value: 'up', child: Text('ปัดขึ้น (เต็มบาท)')),
                            DropdownMenuItem(
                                value: 'down', child: Text('ปัดทิ้ง (เต็มบาท)')),
                          ],
                          onChanged: (val) {
                            if (val != null) {
                      ...
                      ListTile(
                        ...
                        trailing: DropdownButton<String>(
                          ...
                          items: const [
                            DropdownMenuItem(
                                value: 'per_item',
                                child: Text('ลดต่อรายการ (รวม)')),
                            DropdownMenuItem(
                                value: 'per_piece', child: Text('ลดต่อชิ้น')),
                          ],
                          onChanged: (val) {
                            if (val != null) {
                ...
                const Padding(
                  padding: EdgeInsets.only(left: 8.0, bottom: 5),
                  child: Text('ความปลอดภัย (Security Settings)',
