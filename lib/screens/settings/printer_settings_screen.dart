              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSectionHeader('เครื่องพิมพ์บิลเงินสด (POS Slip)',
                      Icons.payments, Colors.green, isLocal: true),
                  ...
                  _buildSectionHeader('เครื่องพิมพ์บิลเงินสด (Full Receipt)',
                      Icons.description, Colors.indigo, isLocal: true),
                  _buildCashBillPrinterCard(),
                  const SizedBox(height: 30),

                  _buildSectionHeader('เครื่องพิมพ์ใบกำกับภาษี (Full Tax)',
                      Icons.receipt_long, Colors.orange, isLocal: true),
                  _buildPrinterCard(
                    printer: _selectedTaxPrinter,
                    onChanged: (p) {
                  ...
                  _buildSectionHeader('เครื่องพิมพ์ใบส่งของ/ลูกหนี้',
                      Icons.local_shipping, Colors.blue, isLocal: true),
                  _buildDeliveryCard(),
                  const SizedBox(height: 10),
                  SwitchListTile(
                    title: const Text(
                  ...
                  _buildSectionHeader('เครื่องพิมพ์สติกเกอร์ (Label Printer)',
                      Icons.label, Colors.teal, isLocal: true),
                  _buildBarcodePrinterCard(),
                  const SizedBox(height: 30),
                  _buildSectionHeader('ลิ้นชักเก็บเงิน (Cash Drawer)',
                      Icons.point_of_sale, Colors.blueGrey, isLocal: true),
                  _buildDrawerCard(),
                  const SizedBox(height: 30),
                  SizedBox(
    ...
    try {
      if (_drawerUsePrinter) {
        ...
        if (_drawerPort.contains('.')) {
          ...
          String logDetail = 'Connecting to IP: $ip Port: $p with bytes: $bytes';
          debugPrint('🚪 [Drawer] $logDetail');
          try {
            ...
            AlertService.show(
              context: context,
              message: 'ทดสอบสำเร็จ: เชื่อมต่อและส่งข้อมูลไปที่ $ip เรียบร้อย',
              type: 'success',
            );
            return;
          } catch (e, stacktrace) {
            logDetail += '\n❌ Error: $e\n$stacktrace';
            debugPrint('🚪 [Drawer] SOCKET ERROR: $logDetail');
            if (!mounted) return;
            showDialog(
              context: context, 
              builder: (ctx) => AlertDialog(
                title: const Text('ข้อผิดพลาดการเตะลิ้นชัก (Error)'),
                content: SingleChildScrollView(child: Text(logDetail)),
                actions: [
          ...
          // Network Shared Printer (UNC Path e.g. \\ms\เครื่องพิมพ์ใบเสร็จ)
          String logDetail = 'Copying bytes to network share: $_drawerPort with bytes: $bytes';
          debugPrint('🚪 [Drawer] $logDetail');
          try {
            ...
            if (result.exitCode == 0) {
              ...
              AlertService.show(
                context: context,
                message: 'ทดสอบสำเร็จ: ส่งคำสั่งลิ้นชักไปที่ $_drawerPort เรียบร้อย',
                type: 'success',
              );
            } else {
              logDetail += '\n❌ Command failed:\n${result.stderr}';
            ...
            showDialog(
              context: context, 
              builder: (ctx) => AlertDialog(
                title: const Text('ข้อผิดพลาดเครือข่าย'),
                content: SingleChildScrollView(child: Text(logDetail)),
                actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('ปิด'))]
              )
            );
          ...
          try {
            final file = File('\\\\.\\$_drawerPort'); // Windows path namespace for devices
            await file.writeAsBytes(bytes);
          } catch (e) {
            debugPrint('🚪 [Drawer] COM ERROR: $e');
            if (!mounted) return;
            showDialog(
              context: context, 
              builder: (ctx) => AlertDialog(
                title: const Text('ข้อผิดพลาดการเตะลิ้นชัก (Error)'),
                content: SingleChildScrollView(child: Text('COM Port Error: $e')),
                actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('ปิด'))]
              )
            );
            return;
