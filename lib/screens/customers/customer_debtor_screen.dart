    ...
    await showDialog(
      ...
      builder: (dialogContext) {
        ...
        return StatefulBuilder(builder: (builderContext, setStateDialog) {
          ...
          double remainingDebt = inputAmount < debtToPay ? debtToPay - inputAmount : 0;
                    ...
                    ? () async {
                        ...
                        try {
                          ...
                          if (_selectedIds.isNotEmpty) {
                            // ✅ แก้ไข: _selectedIds คือ Transaction ID ต้องหา Order ID ก่อน
                            final selectedOrderIds = _ledger
                                .where((item) =>
                            ...
                            // Batch Payment (ตัดบิลที่เลือก)
                            await _debtRepo.processBatchPayment(
                                customerId: _currentCustomer.id,
                                payAmount: inputAmount - change, // Net Amount
                                orderIds: selectedOrderIds);
                          } else {
                            // ✅ ถ้ารับชำระเหมา (ไม่ติ๊ก) -> ดึงบิลเก่าสุดมาตัดอัตโนมัติ
                            final pendingBills = await _debtRepo.getPendingBills(_currentCustomer.id);
