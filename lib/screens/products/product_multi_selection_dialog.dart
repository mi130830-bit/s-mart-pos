...
import '../../utils/barcode_utils.dart'; // ✅ Added for barcode fix
...
class _ProductMultiSelectionDialogState
  ...
  Future<void> _loadProducts({bool reset = false}) async {
    ...
    try {
      var newProducts = await _repo.getProductsPaginatedLight(
        _currentPage,
        _pageSize,
      ...
      // ✅ Barcode Fix Logic
      if (newProducts.isEmpty && _searchQuery.isNotEmpty && BarcodeUtils.isThaiInput(_searchQuery)) {
        final fixedQuery = BarcodeUtils.fixThaiInput(_searchQuery);
        if (fixedQuery != _searchQuery) {
          newProducts = await _repo.getProductsPaginatedLight(
            _currentPage,
            _pageSize,
            searchTerm: fixedQuery,
          );
        }
      }
