  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final received = await _stockRepo.getPurchaseOrders(
        ...
        supplierId: _selectedSupplierId,
        isPaid: isPaidFilter,
        limit: _limit + 1, // Fetch 1 extra to check for next page
      ...
      if (received.length > _limit) {
        hasNext = true;
        received.removeLast(); // remove the extra item
      }
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              ],  // end if(_selectedSupplierId != null)
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(
                          value: 'UNPAID',
                          child: Text('⚠️ ค้างจ่าย', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold))),
                      DropdownMenuItem(
                          value: 'PAID',
                          child: Text('✅ จ่ายแล้ว', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold))),
                    ],
                    onChanged: (val) {
        ...
        if (displayOrders.isEmpty) ...[
              ...
              children: [
                ...
                const Expanded(
                    ...
                    child: Text('ยอดรวม',
                        ...
                        style: TextStyle(
                            color: Colors.white, fontWeight: FontWeight.bold))),
                const SizedBox(width: 96), // แก้ไข + ลบ
              ],
            ),
          ),
          // List Body
            ...
            child: ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                if (i == 0) {
                  ...
                  final prevOrder = displayOrders[i - 1];
                  final prevDt = DateTime.parse(prevOrder['createdAt'].toString());
          ...
          if (_orders.isNotEmpty) ...[
            Builder(builder: (context) {
              ...
              final totalUnpaid = _orders
                  .where((o) => (int.tryParse(o['isPaid']?.toString() ?? '0') ?? 0) == 0)
                  .fold<double>(0, (s, o) => s + (double.tryParse(o['totalAmount']?.toString() ?? '0') ?? 0));
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      '฿${NumberFormat('#,##0.00').format(totalAll)}',
                      style: const TextStyle(
  ...
  Future<void> _togglePaymentStatus(Map<String, dynamic> order) async {
    ...
    try {
      ...
      if (mounted) {
        AlertService.show(
          context: context,
          message: 'ไม่สามารถอัปเดตสถานะได้: $e',
          type: 'error',
        );
      }
    }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final received = await _stockRepo.getPurchaseOrders(
        ...
        startDate: startDate,
        endDate: endDate,
        limit: 1000, // เพิ่ม Limit เป็น 1000 รายการ
      );
      ...
      if (mounted) {
        setState(() {
          ...
          _summaryList.sort((a, b) => (b['totalAmount'] as double).compareTo(a['totalAmount'] as double)); // sort by highest amount
          _isLoading = false;
        });
      }
    } catch (e) {
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                SizedBox(width: 60), // for action icon
              ],
            ),
          ),
          // List Body
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  '฿${NumberFormat('#,##0.00').format(totalGrand)}',
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.green),
                ),
                const SizedBox(width: 60), // alignment padding
              ],
            ),
          ),
        ],
...
// ---------------------------------------------------------------------------
// Supplier Order History Detail Screen
// ---------------------------------------------------------------------------
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (widget.dateFilter != null) {
        startDate = DateTime(widget.dateFilter!.year, widget.dateFilter!.month,
            widget.dateFilter!.day, 0, 0, 0);
        endDate = DateTime(widget.dateFilter!.year, widget.dateFilter!.month,
            widget.dateFilter!.day, 23, 59, 59);
      }
