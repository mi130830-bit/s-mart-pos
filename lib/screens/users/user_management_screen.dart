...
class _UserManagementScreenState extends State<UserManagementScreen> {
  ...
  void _showUserDialog({model.User? user}) {
    ...
    // ✅ Bug Fix: capture setState of dialog so permission load updates dialog, not screen
    ...
    showDialog(
      ...
      builder: (dialogCtx) => StatefulBuilder(
        builder: (builderCtx, setState) {
          // ✅ Wire dialogSetState so async permission load can update this dialog
                      ...
                      GridView.count(
                        ...
                        children: [
                          ...
                          _buildPermToggle('audit_log', 'ดู Log ประวัติ',
                              permissions, setState),
                          // --- Dashboard Tabs ---
                          _buildPermToggle('dashboard_view_summary',
                              'Dash: สรุปยอด', permissions, setState),
                          _buildPermToggle('dashboard_view_trend', 'Dash: กราฟ',
                              permissions, setState),
                          _buildPermToggle('dashboard_view_ai', 'Dash: AI',
                              permissions, setState),
                          _buildPermToggle('dashboard_view_best_selling',
                              'Dash: ขายดี', permissions, setState),

                          // --- Sales History Actions ---
                          _buildPermToggle('history_view_detail',
                              'ประวัติ: ดูบิล', permissions, setState),
                          _buildPermToggle('history_reprint',
                              'ประวัติ: พิมพ์ซ้ำ', permissions, setState),
                          _buildPermToggle('history_send_delivery',
                              'ประวัติ: ส่งขนส่ง', permissions, setState),
                          _buildPermToggle('history_send_pickup',
                              'ประวัติ: ส่งหลังร้าน', permissions, setState),
                          _buildPermToggle('history_delete_bill',
                              'ประวัติ: ลบบิล', permissions, setState),
                          _buildPermToggle('history_edit_customer',
                              'ประวัติ: แก้ไขลูกค้า', permissions, setState),
                          ...
                          _buildPermToggle('delivery_pending', 'รอส่ง/กำลังส่ง',
                              permissions, setState),
                          _buildPermToggle('delivery_report_reminder',
                              'แจ้งเตือนสิ้นเดือน', permissions, setState),
                          _buildPermToggle('delivery_report_export',
                              'Export ขนส่ง', permissions, setState),
              ...
              CustomButton(
                ...
                onPressed: () async {
                  ...
                  final newUser = model.User(
                    ...
                    role: selectedRole,
                    passwordHash: passwordCtrl
                        .text, // Don't trim password here if we want to allow spaces, but for safety usually trim.
                    // Let's trim password too as it's a common issue.
                    isActive: isActive,
                  ...
                  if (isEditing) {
                    ...
                    // Pre-trim password in the object before passing to Repo?
                    // Actually, let's explicit trim in constructor above.
                    // Wait, cannot modify final. newUser created above.
                    // Re-instantiating with trimmed password.
                    final trimmedUser = model.User(
                      id: newUser.id,
                      username: newUser.username,
                      displayName: newUser.displayName,
                      role: newUser.role,
                      passwordHash: passwordCtrl.text.trim(), // Explicit Trim
