...
class BarcodePrintService {
  ...
  final Map<int, int> _printQueue = {};
  final Map<int, Product> _productCache = {};

  Map<int, int> get printQueue => _printQueue;
  Map<int, Product> get productCache => _productCache;

  void updateQueue(int productId, int count, [Product? product]) {
    if (count <= 0) {
      _printQueue.remove(productId);
      // We keep the cache/product info just in case, or we can remove it.
      // Keeping it is safer for "undo" actions or re-adding same item.
    } else {
      _printQueue[productId] = count;
      if (product != null) {
